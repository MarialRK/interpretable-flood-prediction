...
<style>
    ...
    .header-title { font-size: 2.5rem; font-weight: 700; margin-bottom: 0.2rem; }
    .header-subtitle { font-size: 1.2rem; opacity: 0.9; font-weight: 300; }
    .header-location { font-size: 1rem; opacity: 0.8; margin-top: 0.5rem; }
    .feature-box {
        background: #f8f9fa;
        padding: 0.8rem;
        border-radius: 8px;
    ...
    .feature-impact-positive { color: #dc3545; font-weight: 600; }
    .feature-impact-negative { color: #28a745; font-weight: 600; }
    .feature-impact-neutral { color: #6c757d; }
    .rec-card {
        padding: 0.8rem 1rem;
        border-radius: 8px;
        margin: 0.5rem 0;
...
# ============================================================
# LOAD OR GENERATE BEST MODEL
# ============================================================

@st.cache_resource
def load_best_model():
    """Load the best model and artifacts. Generate if missing."""
    
    model_path = 'models/svm_model.pkl'
    
    # If model doesn't exist, generate it
    if not os.path.exists(model_path):
        st.warning("🔄 Training models for the first time. This may take 2-3 minutes...")
        
        # Run data generation
        if not os.path.exists('processed_data/flood_dataset_large.csv'):
            st.info("📊 Generating dataset...")
            try:
                subprocess.run(['python', 'generate_data.py'], capture_output=True, check=True)
        ...
        try:
            subprocess.run(['python', 'train_svm.py'], capture_output=True, check=True)
        ...
        st.info("🤖 Training ARIMA model for comparison...")
        try:
            subprocess.run(['python', 'train_arima.py'], capture_output=True, check=True)
            st.success("✅ All models trained!")
        except Exception as e:
            st.warning(f"⚠️ ARIMA training note: {e}")
    ...
    try:
        ...
        with open('models/svm_features.pkl', 'rb') as f:
...
with st.sidebar:
    ...
    rainfall_mm = st.slider("🌧️ Rainfall (mm)", min_value=0, max_value=300, value=120,
                            help="Monthly rainfall in millimeters")
    
    rainfall_anomaly = st.slider("📈 Rainfall Anomaly", min_value=-1.0, max_value=2.0, value=0.0, step=0.05,
                                 help="Deviation from average rainfall")
    
    st.markdown("---")
    
    ndvi = st.slider("🌿 NDVI (Vegetation Health)", min_value=0.0, max_value=1.0, value=0.50, step=0.01,
                     help="0 = barren, 1 = dense vegetation")
    
    st.markdown("---")
    
    elevation_m = st.slider("🏔️ Elevation (m)", min_value=390, max_value=470, value=425,
                            help="Elevation above sea level in meters")
    
    slope_pct = st.slider("📐 Slope (%)", min_value=0.0, max_value=0.5, value=0.12, step=0.01,
...
if hasattr(model, 'coef_'):
    ...
    explanation_df = pd.DataFrame({
        ...
        'Abs_Impact': np.abs(contributions)
...
with col1:
    ...
    for _, row in explanation_df.iterrows():
        ...
        if impact > 0.05:
            ...
            cls = "feature-impact-positive"
        elif impact < -0.05:
            color = "🟢"
            direction = "decreases flood risk"
            cls = "feature-impact-negative"
        else:
            color = "⚪"
            direction = "neutral"
            cls = "feature-impact-neutral"
        ...
        <div class="feature-box">
            ...
            <div class="{cls}">{color} {direction}</div>
        </div>
        """, unsafe_allow_html=True)
...
with col2:
    ...
    colors = ['red' if x > 0.05 else 'green' if x < -0.05 else 'gray' for x in explanation_df['Impact']]
...
if pred_class == 1:
    ...
    with col1:
        st.markdown("""
        <div class="rec-card"><strong>📢 Early Warning</strong>
        <p style="margin:0.3rem 0 0 0;font-size:0.9rem;">Alert local authorities and communities</p></div>
        """, unsafe_allow_html=True)
    with col2:
        st.markdown("""
        <div class="rec-card"><strong>🐄 Livestock</strong>
        <p style="margin:0.3rem 0 0 0;font-size:0.9rem;">Relocate animals to higher ground</p></div>
        """, unsafe_allow_html=True)
    with col3:
        st.markdown("""
        <div class="rec-card"><strong>📦 Supplies</strong>
        <p style="margin:0.3rem 0 0 0;font-size:0.9rem;">Stockpile emergency food and medicine</p></div>
        """, unsafe_allow_html=True)
...
if comparison_df is not None and len(comparison_df) > 0:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=comparison_df['Model'], y=comparison_df['Accuracy']*100, name='Accuracy', marker_color='#2d6a9f'))
    fig.add_trace(go.Bar(x=comparison_df['Model'], y=comparison_df['F1-Score']*100, name='F1-Score', marker_color='#ff6b6b'))
    fig.update_layout(title='Model Comparison', xaxis_title='Model', yaxis_title='Score (%)',
                      yaxis_range=[0, 100], barmode='group', height=400, template='plotly_white')
    ...
    with col2:
        ...
        <div style="background:#d4edda;padding:1.5rem;border-radius:10px;text-align:center;">
            ...
            <div style="font-size:1.5rem;font-weight:700;color:#155724;">{best_model_name}</div>
...
with col1:
    st.markdown("""<div style="background:#f8f9fa;padding:1rem;border-radius:10px;text-align:center;">
        <div style="font-size:1.5rem;">📡</div><div style="font-weight:600;">Data Sources</div>
        <div style="font-size:0.8rem;color:#6c757d;">CHIRPS • MODIS • SRTM • OCHA</div></div>""", unsafe_allow_html=True)

with col2:
    st.markdown("""<div style="background:#f8f9fa;padding:1rem;border-radius:10px;text-align:center;">
        <div style="font-size:1.5rem;">🤖</div><div style="font-weight:600;">Algorithms</div>
        <div style="font-size:0.8rem;color:#6c757d;">XGBoost • RF • SVM • LSTM • GRU • ARIMA</div></div>""", unsafe_allow_html=True)

with col3:
    st.markdown("""<div style="background:#f8f9fa;padding:1rem;border-radius:10px;text-align:center;">
        <div style="font-size:1.5rem;">🔍</div><div style="font-weight:600;">Explainability</div>
        <div style="font-size:0.8rem;color:#6c757d;">SHAP Feature Attribution</div></div>""", unsafe_allow_html=True)

with col4:
    st.markdown("""<div style="background:#f8f9fa;padding:1rem;border-radius:10px;text-align:center;">
        <div style="font-size:1.5rem;">📍</div><div style="font-weight:600;">Region</div>
        <div style="font-size:0.8rem;color:#6c757d;">Sudd Wetland, South Sudan</div></div>""", unsafe_allow_html=True)
